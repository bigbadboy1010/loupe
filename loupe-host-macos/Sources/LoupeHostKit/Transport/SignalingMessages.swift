...
/// Codable mirror of the wire protocol defined in `loupe-signaling/src/signaling/messages.ts`.
/// Kept deliberately in sync with the server; both ends must agree on the shape.
...
public enum InboundSignal: Sendable {
    ...
    public static func decode(from data: Data) throws -> InboundSignal {
        ...
        case "peer-left":
            return .peerLeft
        case "offer":
            return .offer(try require(envelope.payloadSdp))
        case "answer":
            return .answer(try require(envelope.payloadSdp))
        case "ice":
            return .ice(try require(envelope.payloadIce))
        case "turn-cred":
            return .turnCred(iceServers: envelope.iceServers ?? [], ttlSeconds: envelope.ttlSeconds ?? 0)
        case "error":
            return .error(code: envelope.code ?? "INTERNAL", message: envelope.message ?? "")
        default:
            throw SignalingDecodeError.unknownType(envelope.type)
        }
    }
