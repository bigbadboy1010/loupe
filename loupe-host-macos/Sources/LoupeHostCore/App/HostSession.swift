...
public final class HostSession: EncodedFrameSink, @unchecked Sendable {
    ...
    private func wirePeerCallbacks(injector: InputInjector) {
        peer.onInputEvent = { [weak self] event in
            ...
            if Self.isKeyboardEvent(event) { self.keyboardEventCount += 1 }
            if Self.isScrollEvent(event) { self.scrollEventCount += 1 }
            let applied = injector.apply(event)
            if !applied {
                self.inputIgnoredCount += 1
                self.log("accessibility permission missing or input unsupported; input ignored by macOS count=\(self.inputIgnoredCount)")
            }
            if self.inputEventCount <= 5 || self.inputEventCount % 25 == 0 || Self.isKeyboardEvent(event) || Self.isScrollEvent(event) {
    ...
    private static func describe(_ event: InputEvent) -> String {
        ...
        case let .mouseDelta(deltaX, deltaY):
            return "mouseDelta dx=\(format(deltaX)) dy=\(format(deltaY))"
        ...
        case let .textInput(text):
            return "textInput length=\(text.count)"
        }
    }
