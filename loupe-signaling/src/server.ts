...
import { registerSite, resolveSiteDir } from "./site/router.js";
import { LoggingMailer } from "./waitlist/mailer.js";
...
export async function buildServer(config: AppConfig): Promise<FastifyInstance> {
  ...
  if (config.SERVE_SITE) {
    const siteDir = config.SITE_DIR ?? resolveSiteDir();
    const waitlistFile =
      config.WAITLIST_FILE ?? path.resolve(process.cwd(), "data", "waitlist.jsonl");
    const token = config.WAITLIST_ADMIN_TOKEN;
    await registerSite(app, {
      siteDir,
      waitlistFile,
      waitlistMailer: new LoggingMailer(app.log),
      trustProxy: true,
    ...
    app.log.info(
      {
        siteDir,
        waitlistFile,
        adminEnabled: Boolean(config.WAITLIST_ADMIN_TOKEN),
      },
      "Public site + waitlist enabled",
    );
  }
