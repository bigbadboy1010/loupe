...
const envSchema = z.object({
  ...
  /**
   * When `true`, the server serves the public marketing site (index, css, js, privacy,
   * imprint) on `/` and accepts waitlist signups on `POST /waitlist`. The signaling
   * protocol routes (`/healthz`, `/pairing`, `/ws`) continue to work unchanged.
   */
  SERVE_SITE: z
    .enum(["true", "false"])
    .default("false")
    .transform((v) => v === "true"),
  /** Directory holding the compiled static site. Defaults to `<dist>/site`. */
  SITE_DIR: z.string().min(1).optional(),
  /** File path for the append-only JSONL waitlist store. */
  WAITLIST_FILE: z.string().min(1).optional(),

  /**
   * When set, enables `GET /admin/waitlist.csv` and `GET /admin/waitlist.json`.
   * Required for both. Must be at least 32 characters. The CSV endpoint streams the
   * full waitlist so the operator can mail users when the beta opens.
   */
  WAITLIST_ADMIN_TOKEN: z.string().min(32).optional(),
});
