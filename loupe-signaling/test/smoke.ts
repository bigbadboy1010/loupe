...
async function run(): Promise<void> {
  ...
  const hostOffer = once(host);
  controller.send(
    JSON.stringify({ type: "offer", sessionId, payload: { type: "offer", sdp: "v=0 fake-sdp" } }),
  );
  const relayedOffer = await hostOffer;
  assert.equal(relayedOffer.type, "offer");
  assert.deepEqual(relayedOffer.payload, { type: "offer", sdp: "v=0 fake-sdp" });
