...
struct RemoteScreenView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .simultaneousGesture(twoFingerTapGesture(in: rect))
    ...
    private var statusBadge: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(statusText)
                .font(.caption.bold())
            Text("Frames: \(model.diagnostics.videoFramesReceived) • ICE: \(model.diagnostics.iceConnectionState) • DC: \(model.diagnostics.dataChannelState)")
                .font(.caption2)
                .foregroundStyle(.secondary)
            Text("Input: \(model.diagnostics.inputEventsSent)/\(model.diagnostics.inputEventsAttempted) sent")
                .font(.caption2)
