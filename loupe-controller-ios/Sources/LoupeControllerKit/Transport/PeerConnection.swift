...
public protocol PeerConnection: AnyObject, Sendable {
    ...
    /// - Returns: `true` when libwebrtc accepted the payload for sending; `false` when the data channel is not open or encoding failed.
    @discardableResult
    func sendInput(_ event: InputEvent) -> Bool
