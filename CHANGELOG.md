...
### Sprint 21: Software Bill of Materials + dependency audit (2026-06-24)

Adds the SBOM + dependency-audit pipeline. The SBOM is
generated from the package managers we already use (no
new tools), the audit wraps `npm audit` with a configurable
fail threshold, and the CI workflow runs both on every
push plus a weekly Monday 06:00 UTC schedule for catching
newly-disclosed CVEs.

**New files (Sprint 21):**
- `scripts/sbom-generate.sh` (140 lines) — generates
  CycloneDX 1.5 JSON for all four Loupe surfaces
  (relay, host, controller-ios, controller-mac) plus a
  combined aggregate. Validated as well-formed JSON.
- `scripts/dep-audit.sh` (140 lines) — runs `npm audit`
  for the relay + `swift package show-dependencies` for
  the SwiftPM surface; emits `build/audit.json`; exits
  non-zero on advisories at or above the `--fail-on`
  threshold (default: `high`).
- `.github/workflows/sbom.yml` — 2-job matrix:
  `sbom` (every push + tag + weekly Mon 06:00 UTC,
  uploads the SBOM as a 365-day artifact on release
  tags) and `audit` (every push, fails on high/critical,
  uploads the 90-day report).
- `docs/SBOM.md` — design note. Why CycloneDX 1.5, why
  no new tooling, current dependency posture, how to run
  locally, CI matrix, license inventory.

**Verified locally:**
- `scripts/sbom-generate.sh` → 5 valid `.cdx.json` files
  in `build/sbom/` (combined, relay, host, controller-ios,
  controller-mac)
- `scripts/dep-audit.sh --fail-on=high` → 0 vulnerabilities,
  audit PASSED, `build/audit.json` written
- `bash -n scripts/sbom-generate.sh` → OK
- `bash -n scripts/dep-audit.sh` → OK
- `python3 -c "import yaml; yaml.safe_load(...)" .github/workflows/sbom.yml` → OK

**Current dependency posture:**
- Relay production: 4 deps (`@fastify/websocket`, `fastify`,
  `pino`, `zod`), all MIT/Apache-2.0, all from npm
- Relay dev: 5 deps (`@types/node`, `tsx`, `typescript`,
  `ws`, `@types/ws`)
- SwiftPM: 1 dep (`WebRTC` 120.0.0, BSD-3, stasel/WebRTC
  mirror of Google WebRTC M120) shared by host + controller-ios
  + controller-mac

**Why no new tooling (intentional):**
- The script reads the same `package.json` and
  `Package.swift` files the package managers just parsed,
  so the SBOM cannot drift from the lock file.
- Sprint 21.1 will add `swift package show-dependencies`
  parsing for transitives if a CVE ever hits a transitive
  SwiftPM dep.

**NOT changed (intentionally):**
- The `build/sbom/` and `build/audit.json` artefacts are
  git-ignored; they are re-generated by the CI workflow on
  every push.
- We do NOT pin dev tooling versions beyond what
  `package-lock.json` already pins; the SBOM script
  reflects whatever the lock file resolves today.
